...
const CodeMode = ({ mode, codeResultShown, setCodeResultShown }) => {
  ...
  const handleCodeSubmit = async () => {
    ...
    if (!question) {
      // Replaced alert with console.error or a custom message box for better UX in an iframe
      console.error('Please enter a question about the code.');
      // You might want to display a user-friendly message in the UI instead of an alert
      // For example, set a state variable for an error message and display it conditionally.
      return;
    }
    ...
    try {
      const res = await fetch(`/tools/${endpoint}`, {
        method: "POST",
      ...
      setFormattedResponse(parseFormattedText(data.response));
      setCodeResultShown(true);
    } catch (err) {
      const errorMessage = "Error: Could not connect to server.";
      setResponse(errorMessage);
      setFormattedResponse(parseFormattedText(errorMessage));
      setCodeResultShown(true);
      console.error(err);
    }
  };
